...
import utils.timeseries_split_tabular as timeseries_split_tabular
...
from utils import getters, utils_gluonts
from utils.metrics_by_level import calculate_log_errors
...
class SklearnForecaster(BaseEstimator, RegressorMixin):
    def __init__(
        self,
        cfg_model: configs.SKLearnModel,
        cfg_train=None,
        cfg_dataset=None,
        ...
        self.target = cfg_model.target
        self.date_col = cfg_model.date_col
        self.error_metrics = cfg_model.error_metrics
        self.features = cfg_model.features
        self.cv_config = cfg_model.cv_config

        self.model = hydra.utils.instantiate(cfg_model.model_config, _convert_="all")
        print(self.model)
        self.error_funcs = [
        ...
        self.dict_space = {
            space.name: space for space in models_spaces[cfg_model.model_config["_target_"]]
        }

        if cfg_model.model_config == ["_target_"]:
            logger.warning("Consider searching for params using searchcv method")

    def train(self, input_data: pd.DataFrame, y=None):
        df_pred, df_feat_imp = self.evaluate_CV_timeseries(input_data)
        errs, df_err = calculate_log_errors(
            df=df_pred,
            target=self.cfg_model.target,
            metrics=self.cfg_model.error_metrics,
            # output_dir=hydra_output_dir,
        )
        errs = {"val_" + k: v for k, v in errs.items()}
        logger.info(errs)
        mlflow.log_metrics(get_mean_metrics(errs))

        pd.DataFrame(df_feat_imp).to_html("feat_imp.html")
        mlflow.log_artifact("feat_imp.html", "feature_importance")
        if len(self.features) == 0:
            self.features = input_data.columns.difference([self.target])
            self.model.fit(input_data[self.features], input_data[self.target])

    def predict(self, test_dataset: pd.DataFrame, validation=None):

        X_out = test_dataset.copy()
        X_out["y_pred"] = np.clip(self.model.predict(test_dataset[self.features]), 0, np.inf)
    ...
    def evaluate_CV_timeseries(self, X):
        ...
        X_out = X.copy()
        print(X.columns)
        df_pred, feature_importance = timeseries_split_tabular.cv_timeseries(
    ...
    def evaluate(self, test_dataset: pd.DataFrame, y=None) -> pd.DataFrame:
        X_out = test_dataset.copy()
        y_pred = self.predict(X_out)
        y_true = X_out[self.target]
        X_score = pd.concat([y_true, y_pred], axis=1)
        errs, df_err = calculate_log_errors(
            df=X_score,
            target=self.target,
